...
subdir   = 'simnibs_simulation/mni_volumes'
...
masterte = 'mni_icbm152_t1_tal_nlin_asym_09c_2mm.nii.gz'
...
groupte = 'rest_intra_mask_mni_gm_060.nii.gz'
...
maths = MultiImageMaths()
maths.inputs.in_file       = outdir + 'tms_mPFC_mni_2mm_mask_%i.nii.gz' % (threshol)
maths.inputs.op_string     = '-mul %s'
maths.inputs.operand_files = groupfi
maths.inputs.out_file      = outdir + 'tms_mPFC_mni_2mm_mask_%i_on_gradient.nii.gz' % (threshol)
print(maths.cmdline)
maths.run()
